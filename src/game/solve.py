...
class LevelSolver:
        ...
        while queue:
            ...
            for move in self.possible_moves():
                ...
                if next_state not in visited:
                    ...
                    next_cost = cost + 1
                    heapq.heappush(queue, (next_cost + self.heuristic(next_state), next_cost, next_state, path + [move]))
    ...
    def heuristic(self, state):
        player, boxes = state
        return sum(min(abs(box[0] - goal[0]) + abs(box[1] - goal[1]) for goal in self.goals) for box in boxes)

    def get_goals(self):
        goals = []
        for y, row in enumerate(self.level.grid):
            for x, cell in enumerate(row):
                if cell == 2:  # Assuming 2 represents the goal positions
                    goals.append((x, y))
        return goals

if __name__ == "__main__":
    level = Level.from_file(Paths.LEVELS / "grid.txt")  # Change "grid.txt" to the appropriate file path
